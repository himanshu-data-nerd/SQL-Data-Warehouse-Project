...
Script Purpose:
  ...
  Actions Performed:
    ...
    - Insert transformed and cleansed table from Bronze into Silver tables.
	...
	BEGIN TRY
		...
		SELECT
			...
			CASE 
				...
				ELSE sls_sales
			END AS sls_sales,    --Recalculates sales if the original value is missing or incorrect
			sls_quantity,
			CASE 
		...
		--Loading silver.erp_PX_CAT_G1V2s
		SET @start_time = GETDATE();
